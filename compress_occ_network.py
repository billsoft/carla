#!/usr/bin/env python3
"""
代码压缩脚本 - 专用于压缩 occ_network 目录供 AI 分析
忽略 Markdown 文件 (.md)

用法:
    python compress_occ_network.py
"""
...
def get_file_tree(root_dir, exclude_patterns=None):
    ...
    if exclude_patterns is None:
        exclude_patterns = ['__pycache__', '.git', '.pytest_cache', '*.pyc', '*.pyo',
                           '*.egg-info', 'outputs', 'checkpoints', '*.pth', '*.ckpt', 'wandb', '*.md']
        ...
        for pattern in exclude_patterns:
            if '*' in pattern:
                 ...
                 if pattern.startswith('*') and path.name.endswith(pattern[1:]):
                     return True
            elif pattern in path_str or path.name.startswith('.'):
                return True
        return False
...
def collect_code_files(root_dir, include_exts=None):
    ...
    for file_path in sorted(root_path.rglob('*')):
        ...
        # 显式排除 md (虽然 include_exts 没包含，但双重保险)
...
def main():
    parser = argparse.ArgumentParser(description='压缩 occ_network 代码用于 AI 分析')
    parser.add_argument('--dir', default='occ_network',
                       help='要分析的目录 (默认: occ_network)')
    parser.add_argument('--output', default='occ_network_code_summary.txt',
                       help='输出文件名 (默认: occ_network_code_summary.txt)')
