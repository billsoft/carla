...
**命名者**: Andrej Karpathy (2017-2022 特斯拉 AI 总监)
**首次公开**: 2021 年 Tesla AI Day (视频 1:13:40 开始)

在希腊神话中，**九头蛇（Hydra）** 是一种多头怪兽，砍掉一个头会长出两个新头。Andrej Karpathy 用这个名字命名特斯拉自动驾驶神经网络，寓意：

- **多任务学习**：一个网络同时处理9个任务（就像9个头）
- **特征共享**：所有任务共用一个强大的主干网络（怪兽的身体）
- **鲁棒性强**：即使某个任务失败，其他任务仍能正常工作

**注**: 虽然 Elon Musk 不喜欢这个名字，但它在工程团队内部广泛使用，并成为 FSD 架构的代名词。

### 1.2 核心设计哲学 (Tesla AI Day 2021)

**相机规格** (重要修正!):
- ✅ **分辨率**: 1280×960 (约 1.2MP) - 不是 1920×1080!
- ✅ **色彩深度**: **12-bit RAW** (部分相机14-16bit) - 不是 8-bit!
- ✅ **色彩空间**: YUV 或 RAW Bayer
- ✅ **帧率**: 36 FPS (不是 30 FPS)
- 💡 **设计哲学**: "用深度弥补分辨率" - 高色彩深度保留更多光照信息，等效于在第一层卷积前就已经做了特征提取，大幅减少计算量!
...
```mermaid
graph TB
    subgraph Input["输入层 (Tesla AI Day 2021 规格)"]
        CAM1[前窄角相机<br/>1280×960 12-bit RAW]
        CAM2[前主相机<br/>1280×960 12-bit RAW]
        CAM3[前广角相机<br/>1280×960 12-bit RAW]
        CAM4[左前相机<br/>1280×960 12-bit RAW]
        CAM5[左后相机<br/>1280×960 12-bit RAW]
        CAM6[右前相机<br/>1280×960 12-bit RAW]
        CAM7[右后相机<br/>1280×960 12-bit RAW]
        CAM8[后相机<br/>1280×960 12-bit RAW]
    end
