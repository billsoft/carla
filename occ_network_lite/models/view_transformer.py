...
class EfficientViewTransformer(nn.Module):
    """
    高效 View Transformer
    
    使用 Deformable Attention 减少计算量
    每个 Query 只关注少量采样点，而非所有图像像素
    
    注: 这是简化版，完整版需要 deformable attention 算子
    """
    ...
    def __init__(
        ...
        bev_h: int = 200,
        bev_w: int = 200,
        num_points: int = 8,  # 每个 Query 采样的点数
    ):
        super().__init__()
        ...
        # 参考点（BEV 网格中心归一化到 [0, 1]）
        self.register_buffer(
            'reference_points',
            self._get_reference_points(bev_h, bev_w)
        )
        
        self._init_weights()
        
    def _get_reference_points(self, h: int, w: int) -> torch.Tensor:
        """生成参考点"""
    ...
    def forward(
        ...
        # 这里使用简化的实现，实际应使用 deformable attention
        # 完整实现需要 CUDA 算子支持
        ...
        # 展平图像特征
        img_feat = img_features.permute(0, 1, 3, 4, 2).flatten(1, 3)
        
        # 简单的交叉注意力
        attn_weights = torch.matmul(query, img_feat.transpose(-1, -2))
        attn_weights = F.softmax(attn_weights / (self.embed_dim ** 0.5), dim=-1)
        
        output = torch.matmul(attn_weights, self.value_proj(img_feat))
