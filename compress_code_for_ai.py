#!/usr/bin/env python3
"""
代码压缩脚本 - 用于生成紧凑的单文件给 AI 分析

用法:
    python compress_code_for_ai.py
    python compress_code_for_ai.py --dir occ_network_nano --output code_summary.txt
"""
...
def get_file_tree(root_dir, exclude_patterns=None):
    ...
    if exclude_patterns is None:
        exclude_patterns = ['__pycache__', '.git', '.pytest_cache', '*.pyc', '*.pyo',
                           '*.egg-info', 'outputs', 'checkpoints', '*.pth', '*.ckpt']
...
def collect_code_files(root_dir, include_exts=None):
    ...
    if include_exts is None:
        include_exts = ['.py', '.yaml', '.yml', '.json', '.txt', '.md']

    exclude_patterns = ['__pycache__', '.git', '.pytest_cache', 'outputs',
                       'checkpoints', '.egg-info']
...
def main():
    parser = argparse.ArgumentParser(description='压缩代码用于 AI 分析')
    parser.add_argument('--dir', default='occ_network_nano',
                       help='要分析的目录 (默认: occ_network_nano)')
    parser.add_argument('--output', default='code_summary_for_ai.txt',
                       help='输出文件名 (默认: code_summary_for_ai.txt)')
